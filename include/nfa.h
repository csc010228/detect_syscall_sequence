...
bool NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::new_state(state_index state) {
    ...
    if (this->states_.find(state) == this->states_.end()) {
        ...
        for(auto i : this->transitions_) new_state_transition_table_line.insert(make_pair(i, no_states));
...
bool NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::delete_state(state_index state) {
    if (this->states_.find(state) == this->states_.end() || state == this->start_state_) return false;
    ...
    for (auto i : state_transition_table_) {
        for (auto j : i.second) {
            for (auto k : j.second) {
                if(k == state) remove_path(i.first, j.first, k);
            }
        }
    }
    return true;
...
bool NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::new_transition(AUTOMATA_STATE_TRANSITION transition) {
    ...
    if (this->transitions_.find(transition) == this->transitions_.end()) {
        ...
        for (auto i : state_transition_table_) state_transition_table_.at(i.first).insert(make_pair(transition, no_states));
        return true;
    }
    return false;
}
...
bool NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::remove_path(state_index from_state, AUTOMATA_STATE_TRANSITION transition, state_index to_state) {
    ...
    if (tmp.find(to_state) == tmp.end()) return false;
...
void NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::combine_states(state_index state_1, state_index state_2) {
    ...
    bool tag = false;
    if (state_1 == state_2 || this->states_.find(state_1) == this->states_.end() || this->states_.find(state_2) == this->states_.end()) return;
    for (auto i : state_transition_table_) {
        for (auto j : i.second) {
    ...
    if (state_2 == this->start_state_) this->change_start_state(state_1);
    if (this->is_stop_state(state_2)) this->add_stop_state(state_1);
    if (this->state_datas_.find(state_2) != this->state_datas_.end()) {
        for (auto i : this->state_datas_.at(state_2)) this->add_state_data(state_1, i);
    }
...
set<state_index> NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::get_to_state(state_index from_state, AUTOMATA_STATE_TRANSITION transition) const {
    set<state_index> no_states;
    if (state_transition_table_.find(from_state) == state_transition_table_.end() || state_transition_table_.at(from_state).find(transition) == state_transition_table_.at(from_state).end() || state_transition_table_.at(from_state).at(transition).size() == 0) return no_states;
...
set<state_index> NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::input(AUTOMATA_STATE_TRANSITION transition) {
    ...
    if (current_states_.size() == 0) reboot();
...
map<state_index, state_index> NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::series_connect_NFA(state_index state, NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA> & another_nfa) {
    ...
    for (auto i : another_nfa.states_) {
        if (i != another_nfa.get_start_state()) {
            while (this->states_.find(tmp) != this->states_.end()) tmp++;
            this->new_state(tmp);
            old_new_state_index_map.insert(make_pair(i, tmp));
        } else old_new_state_index_map.insert(make_pair(i, state));
        if (another_nfa.is_stop_state(i)) this->add_stop_state(old_new_state_index_map.at(i));
    }
    for (auto i : another_nfa.state_datas_) {
        for (auto j : i.second) this->add_state_data(old_new_state_index_map[i.first], j);
    }
    for (auto i : another_nfa.state_transition_table_) {
        for (auto j : i.second) {
            for (auto k : j.second) add_path(old_new_state_index_map[i.first], j.first, old_new_state_index_map[k]);
        }
    }
...
DFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA> NFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::to_DFA() const {
    ...
    if (this->is_stop_state(this->start_state_)) res.add_stop_state(dfa_state);
    dfa_state++;
    ...
    for (auto i : old_new_state_index_map) {
        for (auto j : i.first) {
            if (this->state_datas_.find(j) != this->state_datas_.end()) {
                for (auto k : (this->state_datas_).at(j)) res.add_state_data(i.second, k);
            }
        }
    }
    return res;
