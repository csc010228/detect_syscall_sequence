...
bool DFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::new_state(state_index state) {
    ...
    if (this->states_.find(state) == this->states_.end()) {
        ...
        for(auto i : this->transitions_) state_transition_table_.insert(make_pair(state, new_state_transition_table_line));
        return true;
    }
    return false;
}
...
state_index DFA<AUTOMATA_STATE_TRANSITION, RELATED_DATA>::get_to_state(state_index from_state, AUTOMATA_STATE_TRANSITION transition) const {
    if (state_transition_table_.find(from_state) == state_transition_table_.end() || state_transition_table_.at(from_state).find(transition) == state_transition_table_.at(from_state).end()) return this->start_state_;
