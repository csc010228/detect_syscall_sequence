...
struct syscall_probe
{
    ...
    {
        if(string_start_with(probe_name,"tracepoint:syscalls:sys_enter_"))
        {
            type=syscall_probe_type::ENTER;
        }
        else if(string_start_with(probe_name,"tracepoint:syscalls:sys_exit_"))
        {
            type=syscall_probe_type::EXIT;
        }
    };
    ...
    void add_transition(state_index from_state,struct syscall_detection_point * transition,state_index to_state,string stop_action,bool push_to_back)
    {
        if(push_to_back)
        {
            transitions_and_stop_actions.push_back(make_pair(state_transition(from_state,transition,to_state),stop_action));
        }
        else
        {
            transitions_and_stop_actions.push_front(make_pair(state_transition(from_state,transition,to_state),stop_action));
        }
    }
    ...
    string to_string(enum syscall_detection_atom detection_atom)
    {
        string res,action,atom,to_state_str;
        bool first_tag=true;
        size_t index=0;

        if(type==syscall_probe_type::EXIT && transitions_and_stop_actions.size()==0)
        {
            return "";
        }
        ...
        for(auto i:transitions_and_stop_actions)
        {
            ...
            action.clear();
            for(auto j:i.first.transition->action)
            {
                action+=j;
            }
            res+=("if("+atom+"=="+std::to_string(i.first.from_state));
            if(i.first.transition->condition.size()>0)
            {
                res+=(" && ("+i.first.transition->condition+")");
            }
            res+=(")\n\t{\n");
            if(action.size()>0)
            {
            ...
            {
                to_state_str=std::to_string(i.first.to_state);
            }
...
private:
    ...
    //输入系统调用序列检测的脚本语言文件
    bool input_source_file(string source_filename);

    //输出翻译过后的bpftrace脚本语言文件
    bool output_bpftrace_file(string bpftrace_filename);
...
public:
    ...
    bool translate(string source_filename,string bpftrace_filename);
};
